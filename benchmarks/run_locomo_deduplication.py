...
async def ingest_conversation(
    ...
    """Ingest assertions from a past locomo run with own deduplication"""
    ...
    for (msg_id, msg_ingestion), msg_metadata in tqdm(zip(ingestion_logs.items(), conversation_metadata), desc="Extracting memories", leave=True):
        ...
        await memory.add_user_msg(
            ...
            timestamp=timestamp,
            msg_id=msg_id,
            assertions=msg_ingestion["assertions"], # Including assertions here already will make memory orchestrator skip assertion extraction step
        )
